...
CREATE TABLE `__new_rate_plans` (
	...
	CONSTRAINT "rate_plans_daily_cap_nonnegative" CHECK("__new_rate_plans"."daily_cap_cop" is null or "__new_rate_plans"."daily_cap_cop" >= 0),
...
INSERT INTO `__new_rate_plans`("id", "name", "vehicle_type", "billing_unit", "amount_cop", "minimum_charge_cop", "daily_cap_cop", "grace_minutes", "status", "created_at", "updated_at") SELECT "id", "name", "vehicle_type", "billing_unit", "amount_cop", 0, NULL, NULLIF("grace_minutes", 0), "status", "created_at", "updated_at" FROM `rate_plans`;--> statement-breakpoint
